...
Future<void> _loadServiceShifts() async {
  try {
    ...
    final shifts = await ApiService.fetchServiceShifts(serviceId: selectedServiceId ?? widget.serviceId);
...
void _onServiceChanged(int serviceId) {
  setState(() {
    ...
    selectedServiceName = availableServices.firstWhere((service) => service.id == serviceId).name;
...
Future<void> fetchEastAsiaPackages() async {
  try {
    print('Fetching East Asia packages with professionId: ${widget.professionId}, serviceId: ${selectedServiceId ?? widget.serviceId}');
    ...
    final packages = await ApiService.fetchEastAsiaPackages(
      ...
      serviceId: selectedServiceId ?? widget.serviceId, // Use selectedServiceId
      serviceShift: (selectedServiceId ?? widget.serviceId) == 1 ? selectedEastAsiaShift : null,
    );
...
Future<void> fetchAfricanPackages() async {
  try {
    print('Fetching African packages with professionId: ${widget.professionId}, serviceId: ${selectedServiceId ?? widget.serviceId}');
    ...
    final packages = await ApiService.fetchAfricanPackages(
      professionId: widget.professionId,
      serviceId: selectedServiceId ?? widget.serviceId, // Use selectedServiceId
      serviceShift: (selectedServiceId ?? widget.serviceId) == 1 ? selectedAfricanShift : null,
    );
...
Widget _buildServiceSelector() {
  ...
  if (availableServices.length > 1) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 1, vertical: 0), // Remove vertical padding
      child: Column(
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: availableServices.map((service) => 
                Container(
                  margin: EdgeInsets.only(right: 5), // Horizontal spacing between options
            ...
            SliverToBoxAdapter(
              child: Column(
                children: [
                  ...
                  Container(
                    padding: EdgeInsets.symmetric(horizontal: 20, vertical: 20), // Consistent padding
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
  ...
  return GestureDetector(
    onTap: () {
      ...
      ContinuousBookingOverlay.showAsCustomOverlay(
        context,
        serviceId: selectedServiceId ?? widget.serviceId, // Use selectedServiceId instead of widget.serviceId
