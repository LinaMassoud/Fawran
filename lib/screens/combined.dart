...
class CombinedOrderScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CombinedOrderScreen> createState() => _CombinedOrderScreenState();
}
...
class _CombinedOrderScreenState extends ConsumerState<CombinedOrderScreen> {
  ...
  child: Padding(
    ...
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _infoRow('Driver', ref.read(selectedLaborerProvider)?.arabicName ?? ''),
        const SizedBox(height: 8),
        _infoRow('Nationality', ref.read(selectedLaborerProvider)?.nationality ?? ''),
        _infoRow('Package', ref.read(selectedPackageProvider)?.packageName ?? ''),
        _infoRow('Days', '${ref.read(selectedPackageProvider)?.contractDays ?? ''}'),
        _infoRow('Price', '${ref.read(selectedPackageProvider)?.packagePriceWithVat.toStringAsFixed(2) ?? ''} Riyal'),
      ],
    ),
  ),
)
